...
zxuno: $(SOURCES)
	$(SJASMPLUS) main.asm -DP3DOS -DUNO -DTIMEX --lst=main.lst -DV=$(VERSION)

zxuno-zxscreen: $(SOURCES)
	$(SJASMPLUS) main.asm -DP3DOS -DUNO -DZXSCR --lst=main.lst -DV=$(VERSION)
...
nedoosatm: $(SOURCES)
	$(SJASMPLUS) main.asm -DNEDOOS -DNEDOOSATM -DAUTH -DGS --lst=main.lst -DV=$(VERSION)

nedoosevo: $(SOURCES)
	$(SJASMPLUS) main.asm -DNEDOOS  -DNEDOOSEVO -DAUTH -DGS --lst=main.lst -DV=$(VERSION)
